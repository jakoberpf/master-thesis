...
% TODO red mal mit deinen betreuern ueber den titel, ob der so passt. ich glaube depended (abhaengend) macht mehr sinn wenn es dependent (abhaengig) heisst. und wahrscheinlich correlationS 
\author{
B. Sc. Jakob Erpf\\ 
[1cm]
\small Mentoring:\\ 
...
Notation \ref{formula_pearson} shows the Pearson's $r$ correlation coefficient, which is a direct usage of the definition in \ref{formula_correlation_basic}, assuming that both vectors have the same length, named $i$. The symbols $\bar{x}$ and $\bar{y}$ corresponds to the means of vector $x$ and $y$, respectively. \cite{BenestyChenHuang2009,Zychlinski2018}
\smallskip
\begin{equation}
...
To determine if the differences between the means, represented by the correlation coefficient $r$, are statically significant, a 2-tailed test can be applied. It calculates the $p$-value, which is compared to the significance level $\alpha$, to either accept ($p > \alpha$) or reject ($p <= \alpha$) the null hypothesis, that the means are equal to the population. Usually a value of $0.05$ is chosen for $\alpha$, which means that there is a 5\% risk of rejecting the null hypothesis even if there are differences. From this definition the following two interpretations of the correlation coefficient can be drawn. \cite{OTSD2020}

\begin{itemize}
	\item $p <= \alpha$ means that the null hypothesis can be rejected and indicates that is a significant correlation between the two tested variables. It can be concluded that the increase or decrease of one variabel does significantly related to the increase or decrease of the other.
	\item $p > \alpha$ means that there is \textbf{no} significant correlation between the two variables and no conclusion can be drawn for the correlation.
\end{itemize}

\subsection{Continuous - Nominal}

This type of relation is objectively the most complex to evaluate. One well known method to analyze the relation between a continuous and categorical variable, which is not ranked and has more than to values, is the analysis of variance (ANOVA). The parametric test compares the means of the continuous variable and each group of the nominal variabel, similar to Pearson's $r$. As it is parametric it unfortunately assumes normal or gaussian distributed variables, which is not give in our datasets (see chapter \ref{data}). A non parametric approach of the ANOVA is the rather uncommon Kruskal-Wallis H-test \cite{Leon1998}. Both tests indicates if at least one variabel stochastically dominates another, but not in which groups or in how many groups this domination occurs \cite{OTSD2020}. They therefore do not provide a statement about the correlation strength, but the statistical significance of a possible correlation.

The research of a correlation coefficient for the relation of a continuous and nominal variable only provided one option, the eta ($\eta$) coefficient from Pearson. 
...
\cite{Benninghaus2007}

\subsubsection{Interpretation}

Interpretation of Eta

only values from 0 to 1.


\subsubsection{Significance}
The significancy evaluation for the $\eta$ coefficient is done via the $p$-value of the Kruskal-Wallis H-test, where $\eta$ is deviated from.
...
The p-values of the kruskal-wallis test can be calculated with the chi-squared test ($\chi^2$) from Pearson.
...
The interpretation of the $p$-value can be compared to the 2-tailed test, with some differences in mind. Kruskal-Wallis used medians, instead of means and compares multiple medians of each group to the median of the continuous variabel. 
...
The Pearson's $\chi^2$ test is can be applied to categorical data to
...
% https://statistikguru.de/spss/chi-quadrat-test-unabhaengigkeit/einleitung-6.html

\subsubsection{Cramer’s V}

The Cramer’s V notation, also called Cramer's phi ($\Phi_c$), is a measurement for the relation of two nominal variables and most.

is shown in formula \ref{formula_cramers_v_biased}  $k$ and $r$ are the number of columns and row, respectively. $\varphi$, the phi coefficient, defined by $\frac{{\chi^2}}{n_{ij}}$ includes $\chi^2$, which can be derived from Pearsons chi-squared test as shown in \ref{formula_cramers_v_chi}. \cite{Sheskin1997,Bergsma2013}
\smallskip
\begin{equation}
...
The Interpretation of Cramers $V$ is similar to Pearson's correlation coefficient $r$. According to \cite{Cohen1988} and \cite{Ellis2010}
