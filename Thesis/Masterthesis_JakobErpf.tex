...
To determine if $r$, is statically significant, a chi-square test can be applied. Equation \ref{formula_chi_squared_simplified} shows the chi-squared statistic taken from Wikipedia, cited from Karl Pearson \cite{Pearson1990}.
...
The $\eta$ coefficient, also called correlation ratio measures the relationship between variables based on the sums of squares used in the ANOVA \cite{Lewis2012,Benninghaus2007}. More concrete, $\eta$ is the squared root of ratio between $SS_x$ and $SS_y$ \cite{Shaldehi2013}.
...
\subsubsection{Significance}
The significancy evaluation for the $\eta$ coefficient is done via the $p$-value of the Kruskal-Wallis H-test, where the defined $\eta$ is based on. The p-value can be calculated with the chi-squared test ($\chi^2$), defined in section \ref{correlation_pearson} with Pearson's $r$.
