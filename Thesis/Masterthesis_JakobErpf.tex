...
From this curtailed pool of correlate able and analyze able characteristics all parameters that have a logical significance with causes or effects of an accident will be considered in the analysis. To give an overview about what results can be expected on the basis of the provided dataset the 15 most plausible relevant parameters of the dataset will be elaborated and illustrated. The parameters are also categorized in the variable types, defined in chapter \ref{correlation_variable_types}. The referred diagrams are available as larger prints in the appendix \ref{appendix_baysis} for better readability. 
...
\begin{figure}[H]
	...
	\label{img:baysis_dataset_dist_month_absolute}
\end{figure}
...
\paragraph{Kat}
The accident category, described by the variable 'Kat' (shown in figure \ref{img:appendix_baysis_dataset_Kat}), range from just accident with damaged property to deathly accidents, with the number of deadly accidents having the lowest count. The variable consists of four values, which can be ordered, and is therefore ordinal.
...
\paragraph{Beteil}
The distribution of the number of involved persons (shown in figure \ref{img:appendix_baysis_dataset_Beteil}) shows that more than 96\% of accidents have three or less involved persons. The major share of two involved persons makes up for 56\% and the second biggest of one involved person for 30\% of the total count. This also means the accidents with more than three involved persons are statistically insignificant. Because of the increasing order of values, the variable is of ordinal type.

\paragraph{UArt}
The accident cause type, described by the two aggregated variables UArt1 and UArt2 (shown in figure \ref{img:appendix_baysis_dataset_UArt}), presents two major sets of causes. One being the accidents with waiting, stopping and starting vehicles in the same lane, which describe typical collision accidents during congested traffic. The other being the accidents in the next left or right lane, which describe common lane changing collisions. Accidents with cross traffic, pedestrians or opposite traffic are relatively uncommon. The variable does not show any order and is therefore of nominal type.
...
%\end{itemize}
\paragraph{AUrs}
The summarized distribution of the parameters “AUrs1” and “AUrs2” (shown in figure \ref{img:appendix_baysis_dataset_AUrs}) does clearly that only the first category of “Slippery road condition due to oil” hold a significant share. Because of that any correlation to this parameter would not point to any usable statement and therefore can be neglected for the evaluation.

%\begin{itemize}
%Slippery street due to oil
%Slippery street due to dirt
%Slippery street due to snow or ice
%Slippery strett due to rain
%Slippery strett due to other objects
%Cart rut due to rain, snow or ice
%Other condition of road
%Un regular condition of traffic signs 
%Bad lighting of street
%Bad safty on train crossing
%Visibility issues due to fog
%Visibility issues due to rain or hail
%Visibility issues due to sun or glare
%Crosswind
%Visibility issues due to storm 
%Unsafe roadwork
%Wild animals
%Other animals
%Other obstacles
...
\paragraph{Alkoh}
The Alcohol involvement indication parameter only contains two variables of $1 = yes$ or $unknown$. It reveals that only 1,9\% of accidents have one or more involved persons with measurable blood alcohol. The variable only has two unique values and is therefore dichotomous.
...
%\begin{itemize}
%	   /**
%     * [37,38,39] Special characteristic if the accident location/environment
%     */
%Confusing
%Level crossing
%Pedestrian crossing
...
\paragraph{Lich}
The light situation parameter shown in \ref{img:appendix_baysis_dataset_Lich} does not reveal any features and describes the lighting condition when the accident happened. Because is could be ranked from best to works lighting is is of ordinal type.



\paragraph{Zust}
%    /**
%     * [43,44] The condition of the street
%     * <p>
%     * 0 = Trocken
%     * 1 = Nass/Feucht
%     * 2 = Winterglatt
%     * 3 = Schlüpfrichtkeit (Öl, Dung, Laub usw.)
%     */ 
%     Dry
%Wet
%Ice
...
\paragraph{Fstf}
    /**
     * [116] The lane where the accident happened
     * <p>
     * [1,2,3,...] = first, second, ... lane from the right
     * [S] = Beteiligter benutzte zum Unfallzeitpunkt die zum befahren freigegebene Standspur
     * [E] = Beteiligter benutzte zum Unfallzeitpunkt berechtigt die Einbahnstraße entgegengesetzt
     */
...
\bigskip
\textbf{TODO figure of FCD, non euclidean space}
\bigskip

The vertical axis shows the temporal dimension an is regular scales in 3 minute steps. The horizontal axis represents the spatial extend is scaled in one cell per step. These cell represent the space of one section of the road, m
...
\begin{figure}[h]
	...
	\label{img:appendix_baysis_dataset_Zust 'Zust'}
\end{figure}
...
\begin{figure}[h]
	...
	\label{img:appendix_baysis_dataset_Fstf 'Fstf'}
\end{figure}
...
\begin{figure}[h]
	...
	\label{img:appendix_baysis_dataset_WoTag 'WoTag'}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.4, trim=2cm 6cm 0cm 6cm]{../CorrAnalysis/data/BAYSIS/01_dataset/plots/baysis_dataset_sign_cramers}
