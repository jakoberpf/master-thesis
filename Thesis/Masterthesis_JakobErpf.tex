...
To exclude single cell errors and discard jams, which are too small to have any effect and origin to be considered in the analysis, the length and duration of a jam can be used for filtering. The two-dimensional form (see chapter \ref{dataset_fcd}) of the FCD, allows for an easy filtering of detected jams by two conditions. If $l < l_{min}$ or $t < t_{min}$ is given, $l$ being the maximum spatial extend and $t$ being the maximum temporal extend, the jams should be ignored.
...
For the detection algorithm, which is based on a clustering approach (see chapter \ref{cluster_dbscan}), additionally parameters are needed to consolidate individual but adjacent clusters into jams. 
...
\chapter{Methodology}
\label{methodology}
The central aim of the thesis it to determine if jams characteristics, detected in \acrshort{fcd} and incident characteristics from accidents (\acrshort{baysis}) and roadworks (\acrshort{arbis}) are statistically dependent on each other. Therefore the question to be answered stands as follows:
