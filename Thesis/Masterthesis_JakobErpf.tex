...
\begin{equation}
	t_x^{virtual} = (B_x - A_x) \cdot ( x_{interval} + 1 )
	\label{equation_t_v_time}
\end{equation}

\begin{itemize}
	\setlength\itemsep{0.1em}	
	\item[] $A_x | B_x$ is the $x$ index of point $A | B$
...
\begin{equation}
	t_y^{virtual} = (\frac{y_{A_y}^{length}}{2}  \cdot z_{A_y}^{speed}) + (\frac{y_{B_y}^{length}}{2} \cdot z_{B_y}^{speed}) + \sum_{i}^{A_y + 1,...,B_y - 1} (y_{i}^{length} \cdot z_{i}^{speed})
\end{equation}

\begin{itemize}
	\setlength\itemsep{0.1em}	
	\item[] $A_y | B_y$ is the $y$ index of point $A | B$
	\item[] $y_{length}$ is the length of the link
	\item[] $z_{speed}$ is the speed in the cell
\end{itemize}

\paragraph{Diagonal time/space dimension} The diagonal distance or time/space distance is more complex in concept and calculation. 
