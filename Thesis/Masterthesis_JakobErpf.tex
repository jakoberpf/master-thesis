...
\begin{equation}
	t_{x,A,B}^{art} = (x_B - x_A) \cdot ( x_{interval} + 1 )
...
\begin{equation}
	t_{y,A,B}^{art} = (\frac{y_{A}^{length}}{2}  \cdot z_{A}^{speed}) + (\frac{y_{B}^{length}}{2} \cdot z_{B}^{speed}) + \sum_{i}^{y_A + 1,...,y_B - 1} (y_{i}^{length} \cdot z_{i}^{speed})
\end{equation}
...
\begin{equation}
	t_{x,y,A,B}^{art} = \sqrt{(t_{x,A,B}^{art})^2 + (t_{y,A,B}^{art} \cdot c)^2}
\end{equation}
\begin{equation}
...
Parameter calibration is a vital task when implementing algorithms. For the DBSCAN the neighborhood $\varepsilon$ and $minPoints$ 
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1]{../CorrAnalysis/data/BAYSIS/01_dataset/plots/baysis_dataset_count_multiple01}
