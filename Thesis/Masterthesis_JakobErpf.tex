...
For the Interpretation of Cramers $V$ a adaption to the scale of Pearson's $r$ is necessary, but at the same time is quite controversial. Some studies convert $V$ to the effect size $w$ for an equal measurement to $r$ ($r$ is representable over different sudies), when $V$ is already a measure of effect size by itself. \ref{Baguley2016}
%https://www.researchgate.net/post/Whats_the_reason_for_deriving_Cohens_w_from_Cramers_V_if_the_latter_is_already_a_measure_of_effect_size
...
As shown in equation \ref{formula_cohens_w} \ref{Baguley2016}, the conversion from $V$ to $w$ is similar to the reduction of $\phi$ in the definition of $V$ (see equation \ref{formula_cramers_v_corrected}). This advocates the opinion that the conversion to $w$ is necessary within a single study and a adaption of scale is sufficient \ref{Baguley2016}. With the reduced range of $0$ to $1$, the adapted scale of effect size according to Ellis, will be used for interpreting $\eta$ \cite{Ellis2010,Cohen1988}.
...
The Interpretation of Theil's $U$ follows complete other rules, than the guielines mentioned before.

\begin{itemize}
	\item $U < \: 1$ : The forecasting is better than guessing.
	\item $U = \: 1$ : The forecasting is about as good as guessing.
...
\begin{figure}[h]
    ...
    \foreach \x in {1,...,7}
    	\draw[black!30] (\x,0) -- (\x,6);
    	
   	\foreach \y in {1,...,5}
    	\draw[black!30] (0,\y) -- (8,\y); 
	...
	\coordinate[label={270:$$}] (C) at (6.5,1.5);
	\coordinate[label={180:$A$}] (A) at (1.5,1.5);
	\coordinate[label={30:$B$}] (B) at (6.5,4.5);
...
\begin{figure}[h]
    ...
    % X Axis lines  
    \foreach \x in {1,...,9}
    	\draw[red] (\x,1pt) -- (\x,-3pt)
		node[anchor=north] {\x};
    \foreach \x in {0.7,2.5,4,7,9.3}
...
The vertical axis shows the temporal dimension is regular scales in 3 minute steps. The horizontal axis represents the spatial extend is scaled in one cell per step. These cell represent the space of one section of the road, which can have different lengths and make a direct application of the Euclidian distance invalid. 
