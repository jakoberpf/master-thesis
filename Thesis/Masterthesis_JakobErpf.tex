...
\begin{itemize}
	\item[1.] Mark all points as \textit{unvisited}
	\item[2.] Choose point $p$ randomly from all \textit{unvisited} points.
	\begin{itemize}
		\item[a.] Choose point $p$ randomly from all \textit{unvisited} points and mark $p$ as \textit{visited}.
		\item[b.] Count points in the neighborhood $\varepsilon$ to check if $p$ is core point. If $p$ is core point, create new cluster $C$ and add all directly density-reachable and \textit{unvisited} points. Otherwise mark $p$ as noise.
		\item[c.] Choose point $p'$ randomly from all \textit{unvisited} points of $C$ and mark $p$ as \textit{visited}.
		\item[d.] Count points in the neighborhood $\varepsilon$ to check if $p'$ is core point. If $p$ is core point add all directly density-reachable points, which do not already belong to a cluster, to $C$. Otherwise mark $p$ as noise.
		\item[e.] Repeat c. and d. until there are no \textit{unvisited} points left in $C$.	
	\end{itemize} 
	\item[3.] Repeat 2. until all points are \textit{visited} 
\end{itemize}
...
The clustering algorithm is based on the density of points. This density representation is achieved via the calculation of neighborhood $\varepsilon$, typical the Euclidean distance, which is based on the pythagorean theorem (see equation \ref{formula_euclidean} \cite{Erhard2020}).
...
\begin{figure}
	...
	\coordinate[label={270:$C$}] (C) at (4,1.5);
	\coordinate[label={150:$A$}] (A) at (70:4);
	\coordinate[label={30:$B$}] (B) at (42:10);
	...
	\path (B) -- (P) node[midway,above]{$x$}-- (A) node[midway,above]{$y$} ($($(A)!0.5!(B)$)!0.8cm!90:(B)$) node {$d_{A,B}$}; % You can nest calc syntax!
...
A euclidean distance calculation in this form is only possible in Euclidean spaces, which is not the case with FCD. The space of the FCD is defined by a spatial and a temporal dimension, which are scaled differently as mentioned in section \ref{dataset_fcd}. This made is necessary to find a common unit for 
