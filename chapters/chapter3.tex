...
There can be the special case, where the $p$-value of the correlation coefficient and the general Kruskal-Wallis rank sum test show significant differences, but the pairwise Wilcoxon ranked sum test does not show any significant groups. This unfitting difference between the \textit{global} and \textit{specific} significance appears when variables have differences between the samples, but the differences are not reflected by the groups. The interpretability of these relations is controversial and the correlation will therefore dismissed.
