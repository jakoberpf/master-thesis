...
% The sampling error is the uncertainty created selecting a subset from a population. This can result in poorly distributed variables, which induces a high probability of random correlations based on a small number of samples. A initial review of the variable distributions in the \acrshort{BAYSIS} and \acrshort{ArbIS} dataset, resulted in the rule to neglect sets and correlations based on sample size smaller that .1\,\%. This threshold is seems arguably low, but the high number of datasets in the population and the exploratory approach of this thesis allows such a low threshold. 
...
\todo{Ergaenze Effektstaerke für Mann-Whitney-U, wenn genuegenn Zeit für Implementierung}
