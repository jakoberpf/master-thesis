...
Pure correlation results are not sufficient to prove a relation between variables, especially when using the correlation evidence for predictive purposes. Depending on the sample data, statistical results can be random or biased and therefore need to be examined for their probability of error. This is commonly evaluated via the well known statistical significance. Even thought statistical significance is the common standard procedure for evaluating the probability of error, it became a subject of debate in recent years if statistical significance is to generally used and potential misleading. It is often advocated to additionally used statistical uncertainty instead of only relying on statistical significance. \parencite{Harris2019}
...
To define what the significant, as well the non significant groups are referring in the variable, the statical indicators of count ($n$), mean ($\bar{x}$), standard deviation ($\sigma$), median ($\tilde{x}$), $min$, $max$ and range ($\Delta$) are considered. These a quite common for describing statical data and can be considered as robust. These indicators can be presented as table or visual plot, as shown in \cref{tbl:descriptives_sample,fig:descriptives_sample}.
