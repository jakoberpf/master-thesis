...
This chapter will explain the implemented statical methods and their mathematical definitions. This is to provide an in depth understanding how the results, presented in later chapters are comprised. This also clarify which methods and assumptions are implemented to handle a mixed data-based analysis.
...
To determine if $r$, is statically significant, a chi-square test is generally applied to find the $p$-value, testing the probability of independence (see \autoref{correlation_significance}). 
...
Further research of a non-parametric correlation coefficient only provided one suitable option, the eta ($\eta$) coefficient from Pearson \parencite{Benninghaus2007}, describing the relationship between variables based on the sums of squares used in the ANOVA \parencite{Lewis2012,Benninghaus2007}.
...
The significancy evaluation for the $\eta$ coefficient is generally done via the $p$-value of the Kruskal-Wallis $h$-test, since it is a non-parametric test. It is calculated with the chi-squared test ($\chi^2$), defined in \ref{correlation_significance} \parencite{Filipiak2013}. In the case of both a correlated $\eta$ and a significant $h$-test, the exact related groups need to be determined via a post-hoc test (\autoref{correlation_posthoc}).
...
Due to the mathematical similarity of the Point Biserial to the Pearson's $r$, the general interpretation of Pearson's $r$ in \textit{Interpretation of effect size $|r|$} of \ref{correlation_pearson} can be applied to Point Biserial with some adjustments. The range of the Point Biserial coefficient, from $0$ to $1$ removes the direction of correlation from the interpretation. According to Cohen \parencite{Cohen1988} the following can be used as guidelines for the effect size $r_{pq}$ \parencite{Leblanc2017}.
...
The significancy evaluation of the Point Biserial coefficient is done via the 2-tailed $p$-value, a doubled chi-square test (see \autoref{correlation_significance}).
...
The significancy evaluation of Kendall's $\tau$ coefficient is done via the 2-tailed $p$-value, elaborated in \ref{correlation_significance}.
...
In the case of a relation of a categorical variable, the correlation effect size its significance do not provided any statement on which categories are related. This need to be evaluated with a Post Hoc test. In our case the Post Hoc test is based on two separated test. First the variables will be tested for significancy with a Kruskal-Wallis h-test. 
...
For the Kruskal-Wallis H-test applies the following. The higher value of $h$, the higher the variance between the unique variables. A high $h$ supports a already found correlation but must be tested for significance with $\chi^2$, defined in \autoref{correlation_significance}. If the significance, or $p$-value is larger than the defined $\alpha$, the null hypothesis can be rejected, which means the correlation, or variance between the groups are significant. For the identification of which groups create this significant variance, the following Wilcoxon-Mann-Whitney, sometimes also just called Wilcoxon $T$-test is applied pairwise.
...
The Wilcoxon $T$-test is the non-parametric univariate alternative to the dependent $t$-test and is the recommended test to use when the data violates the assumption of a normal distribution. It exists in two variations the Mann-Whitney U statistic and the Wilcoxon W ranked sum Statistic, which can be considered as equivalent. We will use the Wilcoxon ranked sum test in a pairwise approach to compare all groups with reach other. From the resulting matrix of $p$-values and the analog interpretation with the alpha level (see \autoref{correlation_significance}) the significant groups can be identified.

These identified groups are the considered to having a significantly difference to the other groups and therefore can predictably point to a measurement characteristic from a category or vice versa.
