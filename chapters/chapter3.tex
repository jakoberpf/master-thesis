...
To find out what the significant, as well the non significant groups/differences are relating to in the variable, the descriptives of count ($n$), mean ($\bar{x}$), standard deviation ($\sigma$), median ($\tilde{x}$), $min$, $max$ and range ($\Delta$) are considered and presented as shown in \cref{tbl:descriptives_sample,fig:descriptives_sample}.
...
\begin{figure}[ht]
	...
	%\vspace{-8mm}
\end{figure}
From these the amount of difference between the groups can be deviated. 
