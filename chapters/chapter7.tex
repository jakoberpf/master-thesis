...
The relations of the variable \textit{Street} in the global congestion -  accidents dataset show the following features.
\begin{itemize}
...
\end{itemize}
In accordance with these finding, it can be interpreted that in general the street plays a significant role defining the size and form of a congestion. This is not surprising since the demand on the street, which varies heavily between the streets is on major factor influencing the buildup of a congestion. Non the less it is interesting the the differences are substantial with ranges from 30\,\% to 50\,\% between groups and are replicated over difference variables.

The relation class of \text{Jam Intiator} doesn't show any relevance in the found collections, which is unfortunate, since it it the most promising groups for predictions. 
