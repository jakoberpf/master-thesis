...
\newcommand{\sumcompbarstreet}[2]{
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \addplot table[x expr=\coordindex, y=globalMeanMax] {#1};
            \addplot table[x expr=\coordindex, y=effectorMeanMax] {#1};
            \legend{$\bar{x}_{global}$,$\bar{x}_{effector}$}
            \end{axis}
         \end{tikzpicture}
        \caption{Comparison of descriptives $\bar{x}_{global}$ and $\bar{x}_{effector}$ (\textit{#2})}
        %\vspace{-8mm}
    \end{figure}
}